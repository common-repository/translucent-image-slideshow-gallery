	...
	<div class="wrap">
	  <div class="form-wrap">
		...
		if (isset($_POST['TISG_form_submit']) && $_POST['TISG_form_submit'] == 'yes')
		{
			...
			$TISG_title 	= stripslashes(sanitize_text_field($_POST['TISG_title']));
			$TISG_width 	= stripslashes(intval($_POST['TISG_width']));
			$TISG_height 	= stripslashes(intval($_POST['TISG_height']));
			$TISG_delay 	= stripslashes(intval($_POST['TISG_delay']));
			$TISG_speed 	= stripslashes(intval($_POST['TISG_speed']));
			$TISG_dir 		= stripslashes(sanitize_text_field($_POST['TISG_dir']));
			$TISG_link 		= stripslashes(esc_url_raw($_POST['TISG_link']));
			
			if(!is_numeric($TISG_width) || $TISG_width == 0) { $TISG_width = 200; }
			if(!is_numeric($TISG_height) || $TISG_height == 0) { $TISG_height = 155; }
			if(!is_numeric($TISG_delay) || $TISG_delay == 0) { $TISG_delay = 3000; }
			if(!is_numeric($TISG_speed) || $TISG_speed == 0) { $TISG_speed = 400; }
			
			$TISG_dir_1 	= stripslashes(sanitize_text_field($_POST['TISG_dir_1']));
			$TISG_link_1 	= stripslashes(esc_url_raw($_POST['TISG_link_1']));
			$TISG_dir_2 	= stripslashes(sanitize_text_field($_POST['TISG_dir_2']));
			$TISG_link_2 	= stripslashes(esc_url_raw($_POST['TISG_link_2']));
			$TISG_dir_3 	= stripslashes(sanitize_text_field($_POST['TISG_dir_3']));
			$TISG_link_3 	= stripslashes(esc_url_raw($_POST['TISG_link_3']));
			$TISG_dir_4 	= stripslashes(sanitize_text_field($_POST['TISG_dir_4']));
			$TISG_link_4 	= stripslashes(esc_url_raw($_POST['TISG_link_4']));
	  ...
	  <p class="description"><?php _e('(Note: Dont upload your original images into plugin folder. if you upload the images into plugin folder, you may lose the images when you update the plugin to next version. Thus upload your images in "wp-content/uploads/your-folder/" folder and use the folder path as per the example in "Image Directory" text box.)', 'translucent-image-slideshow-gallery'); ?></p>
